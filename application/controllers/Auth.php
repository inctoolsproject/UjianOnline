...
class Auth extends CI_Controller {
		...
		if ($this->input->post()) {
			...
			if ($student) {
				if (password_verify($data['student_password'],$student->student_password)) {
					...
					$this->message('Oooppss','No.Pendaftaran dan password tidak sesuai, silahkan coba lagi','danger');
					redirect('Auth/exam');	
				}
			} else {
				$this->message('Oooppss','No.Pendaftaran tidak terdaftar, silahkan coba lagi','danger');
				redirect('Auth/exam');
			}
		} else {
	...
	public function registrasi() {
		$this->load->view('auth/registrasiExam');
}
	public function regis_submit($id_student = NULL) {
		if ($this->input->post()) {
			$data = $this->input->post();
			// CHECK NIS //
